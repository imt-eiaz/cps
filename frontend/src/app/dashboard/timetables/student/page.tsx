...
export default function StudentTimetablePage() {
  ...
  const fetchTimetable = async () => {
    try {
      setLoading(true);
      const response = await apiClient.get("/academic/timetables/student/me");
      setPeriods(response.data.data?.periods || []);
      setClassInfo(response.data.data?.class || null);
      setError("");
    } catch (err: any) {
      setError(
        err.response?.data?.message || "Failed to load student timetable",
      );
    } finally {
      setLoading(false);
    }
  ...
  const lookup = useMemo(() => {
    const map: Record<string, StudentPeriod> = {};
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        ...
        <h1 className="text-4xl font-bold text-gray-800">Student Timetable</h1>
        <p className="text-gray-600 mt-2">
          {classInfo
            ? `${classInfo.className}${classInfo.sectionName ? ` - ${classInfo.sectionName}` : ""} (${classInfo.classCode})`
            : "Read-only weekly class schedule"}
        </p>
      </div>
